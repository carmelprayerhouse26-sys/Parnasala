...
async function switchTab(tab) {
    ...
    switch (tab) {
        ...
        case 'database': await renderAdminDatabase(content); break;
    }
}
...
async function loadAdminImages() {
    ...
    try {
        ...
        grid.innerHTML = images.map(img => {
            // Handle both local filenames and full Cloudinary URLs
            const src = img.filename.startsWith('http')
                ? img.filename
                : `/uploads/${img.filename}`;
            return `
            <div class="admin-image-card" id="img-card-${img.id}">
                <img src="${src}" alt="${escapeHtml(img.caption || '')}" loading="lazy"
                     onclick="openLightbox('${src}')">
                <div class="admin-image-caption">${escapeHtml(img.caption || 'No caption')}</div>
                <div class="admin-image-actions">
                    <button class="icon-btn" title="Edit Caption" onclick="editImageCaption(${img.id}, '${escapeHtml((img.caption||'').replace(/'/g,"\\'"))}')">
                        <span class="material-icons-round" style="font-size:0.85rem;">edit</span>
                    </button>
                    <button class="icon-btn danger" title="Delete" onclick="deleteImage(${img.id})">
...
function editImageCaption(imageId, currentCaption) {
    const body = `
        <div class="form-group">
            ...
            <input type="text" class="form-input" id="edit-caption-input"
                   value="${escapeHtml(currentCaption)}" placeholder="Enter caption..." autofocus>
        </div>
        <div style="display:flex; gap:0.75rem; justify-content:flex-end; margin-top:1rem;">
            <button type="button" class="btn btn-secondary btn-sm" onclick="closeModal()">Cancel</button>
            <button type="button" class="btn btn-primary btn-sm" id="save-caption-btn">
                <span class="material-icons-round" style="font-size:1rem;">save</span> Save
            </button>
        </div>
    `;
    openModal('Edit Caption', body);
    setTimeout(() => {
        const btn = $('#save-caption-btn');
        const input = $('#edit-caption-input');
        if (btn && input) {
            btn.onclick = async () => {
                try {
                    await api(`/api/admin/images/${imageId}`, {
                        method: 'PUT',
                        body: { caption: input.value.trim() }
                    });
                    closeModal();
        ...
        const body = `
            <form id="edit-article-form">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex; align-items:center; gap:1rem; margin:1rem 0; color:var(--text-muted);">
                        <div style="flex:1; height:1px; background:var(--border-color); opacity:0.5;"></div>
                        <span style="font-size:0.8rem; text-transform:uppercase; letter-spacing:0.05em;">or paste link</span>
                        <div style="flex:1; height:1px; background:var(--border-color); opacity:0.5;"></div>
                    </div>

                    <div class="form-group" style="margin-top:0.5rem;">
                        <label class="form-label" style="display:flex; align-items:center; gap:0.4rem;">
                            <span class="material-icons-round" style="font-size:1.1rem; color:var(--accent);">link</span>
                            PDF URL / Google Drive Share Link
                        </label>
                        <input type="url" class="form-input" id="edit-art-pdf-url" value="${escapeHtml(article.pdf_url || '')}" placeholder="e.g. https://drive.google.com/file/d/.../view">
                    </div>
                </div>
                ...
                <p style="font-size:0.8rem; color:var(--text-muted); margin-bottom:1rem;">
                    ...
                    Provide at least one: PDF file, PDF URL, or text content
                </p>
            ...
            if (form) {
                form.addEventListener('submit', async (e) => {
                    ...
                    try {
                        ...
                        formData.append('pdf_url', document.getElementById('edit-art-pdf-url').value.trim());
...
function renderAddArticleForm(container) {
            ...
            <form id="add-article-form">
                ...
                <div class="form-group">
                    ...
                    <div class="form-group" style="margin-top:0.5rem;">
                        ...
                        <input type="url" class="form-input" id="add-art-pdf-url" placeholder="e.g. https://drive.google.com/file/d/.../view">
                    </div>
                </div>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const pdfUrl = document.getElementById('add-art-pdf-url').value.trim();
            const pdfFile = document.getElementById('add-art-pdf-file');
            const hasPdf = (pdfFile && pdfFile.files.length > 0) || pdfUrl;

            if (!content && !hasPdf) {
                showToast('Please provide text content, a PDF file, or a PDF URL', 'error');
                btn.disabled = false;
                return;
            }
...
// ══════════════════════════════════════════════════════════════════════════════
// TAB: DATABASE (Export / Import)
// ══════════════════════════════════════════════════════════════════════════════

async function renderAdminDatabase(container) {
    // Count stats first
    let stats = { songs: '—', images: '—', categories: '—', articles: '—' };
    try {
        const songs = await api('/api/songs');
    ...
    container.innerHTML = `
        <div style="max-width:700px;">
            <h3 style="margin-bottom:1.5rem; display:flex; align-items:center; gap:0.5rem;">
                <span class="material-icons-round" style="color:var(--accent);">storage</span>
                Database Management
            </h3>
            ...
            <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:1rem; margin-bottom:2rem;">
                ${[
                    ['library_music', stats.songs, 'Songs'],
                    ['label',         stats.categories, 'Categories'],
                    ['photo_library', stats.images, 'Images'],
                    ['auto_stories',  stats.articles, 'Articles']
                ].map(([icon, count, label]) => `
                    <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:var(--radius-md); padding:1.25rem; text-align:center;">
                        <span class="material-icons-round" style="color:var(--accent); font-size:1.8rem;">${icon}</span>
                        <div style="font-size:1.8rem; font-weight:700; color:var(--text-primary); margin:0.25rem 0;">${count}</div>
            ...
            <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:var(--radius-md); padding:1.5rem; margin-bottom:1.5rem;">
                <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.75rem;">
                    <span class="material-icons-round" style="color:#22c55e; font-size:1.4rem;">download</span>
                    <h4 style="margin:0;">Export Database</h4>
                </div>
                <p style="color:var(--text-muted); font-size:0.88rem; margin-bottom:1rem;">
                    Download a complete JSON backup of all songs, images, categories, articles, and settings.
                    Store it safely to restore later.
                </p>
                <button class="btn btn-primary" id="export-db-btn">
                    <span class="material-icons-round" style="font-size:1rem;">cloud_download</span>
                    Download Backup
                </button>
            ...
            <div style="background:var(--bg-card); border:1px solid var(--border-color); border-radius:var(--radius-md); padding:1.5rem; margin-bottom:1.5rem;">
                <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.75rem;">
                    <span class="material-icons-round" style="color:#f59e0b; font-size:1.4rem;">upload</span>
                    <h4 style="margin:0;">Import / Restore</h4>
                </div>
                <p style="color:var(--text-muted); font-size:0.88rem; margin-bottom:0.25rem;">
                    Restore from a previously exported JSON backup. Existing songs are updated; new ones are added.
                    Settings and categories are merged.
                </p>
                <p style="color:var(--error, #ef4444); font-size:0.8rem; margin-bottom:1rem;">
                    ⚠ This will overwrite matching records. Back up your data first.
                </p>
                <div class="file-upload-area" id="import-drop-area" style="padding:1.5rem;">
                    <input type="file" id="import-file" accept=".json" style="display:none;">
                    <span class="material-icons-round">upload_file</span>
                    <p>Click or drag a backup .json file here</p>
                </div>
                <div id="import-status" style="margin-top:0.75rem;"></div>
            </div>

            <!-- Danger Zone -->
            <div style="background:rgba(239,68,68,0.05); border:1px solid rgba(239,68,68,0.3); border-radius:var(--radius-md); padding:1.5rem;">
                <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.75rem;">
                    <span class="material-icons-round" style="color:#ef4444; font-size:1.4rem;">warning</span>
                    <h4 style="margin:0; color:#ef4444;">Danger Zone</h4>
                </div>
                <p style="color:var(--text-muted); font-size:0.88rem; margin-bottom:1rem;">
                    These actions are irreversible. Always export a backup before proceeding.
                </p>
                <div style="display:flex; gap:0.75rem; flex-wrap:wrap;">
                    <a href="/api/admin/db/export" download class="btn" id="export-raw-btn"
                       style="background:rgba(34,197,94,0.15); border:1px solid rgba(34,197,94,0.4); color:#22c55e; font-size:0.85rem;">
                        <span class="material-icons-round" style="font-size:0.9rem;">save_alt</span>
                        Export Full Backup
                    </a>
                </div>
            </div>
    ...
    // Export button
    const exportBtn = $('#export-db-btn');
    if (exportBtn) {
        exportBtn.addEventListener('click', async () => {
            const status = $('#export-status');
            exportBtn.disabled = true;
            exportBtn.innerHTML = '<span class="material-icons-round" style="font-size:1rem;">hourglass_empty</span> Exporting...';
            try {
                ...
                if (status) status.innerHTML = '<p style="color:#22c55e; font-size:0.85rem;">✓ Backup download started</p>';
            } catch (err) {
                if (status) status.innerHTML = `<p class="form-error">${escapeHtml(err.message || 'Export failed')}</p>`;
            } finally {
                exportBtn.disabled = false;
                exportBtn.innerHTML = '<span class="material-icons-round" style="font-size:1rem;">cloud_download</span> Download Backup';
            }
        });
    }
    ...
    if (dropArea && importFile) {
        ...
        dropArea.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files.length > 0) doImport(e.dataTransfer.files[0]);
        });
        importFile.addEventListener('change', () => {
            if (importFile.files.length > 0) doImport(importFile.files[0]);
        });
    }

    async function doImport(file) {
        if (!file.name.endsWith('.json')) {
            importStatus.innerHTML = '<p class="form-error">Please select a .json backup file</p>';
            return;
        }
        confirmDialog(`Import from "${file.name}"? Matching records will be updated.`, async () => {
            importStatus.innerHTML = '<p style="color:var(--accent);">Importing… please wait</p>';
            const formData = new FormData();
            formData.append('file', file);
            try {
                const result = await api('/api/admin/db/import', { method: 'POST', body: formData });
                const { imported } = result;
                importStatus.innerHTML = `
                    <div style="background:rgba(34,197,94,0.1); border:1px solid rgba(34,197,94,0.3); border-radius:8px; padding:1rem; margin-top:0.5rem;">
                        <p style="color:#22c55e; font-weight:600; margin-bottom:0.5rem;">✓ Import complete</p>
                ...
                invalidateCache();
                // Reload tab to show fresh stats
                setTimeout(() => switchTab('database'), 1500);
            } catch (err) {
                importStatus.innerHTML = `<p class="form-error">${escapeHtml(err.message || 'Import failed')}</p>`;
