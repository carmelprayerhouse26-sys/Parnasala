...
@app.route('/api/admin/images/<int:image_id>', methods=['PUT'])
@admin_required
...
def edit_article(article_id):
    ...
    if request.content_type and 'multipart/form-data' in request.content_type:
        ...
        pdf_url = request.form.get('pdf_url', article['pdf_url'] or '').strip()
...
def export_database():
    ...
    settings = dict(conn.execute("SELECT * FROM settings WHERE id=1").fetchone() or {})
...
@admin_required
def import_database():
    """Restore data from a JSON backup. Merges (does not wipe) by default."""
    if 'file' not in request.files:
        return jsonify({'error': 'No file provided'}), 400
    ...
    conn = get_db()
    imported = {'songs': 0, 'categories': 0, 'images': 0, 'articles': 0}

    # Import categories first (songs depend on them)
    ...
    # Import songs (use slug as unique key)
    for song in data.get('songs', []):
        try:
            existing = conn.execute("SELECT id FROM songs WHERE slug = ?", (song.get('slug', ''),)).fetchone()
            if existing:
                conn.execute(
                    "UPDATE songs SET title=?, title_te=?, title_en=?, lyrics=?, lyrics_en=?, category=? WHERE slug=?",
                    (song.get('title',''), song.get('title_te',''), song.get('title_en',''),
                     song.get('lyrics',''), song.get('lyrics_en',''), song.get('category','General'),
                     song['slug'])
                )
            else:
                conn.execute(
    ...
    for art in data.get('articles', []):
        try:
            existing = conn.execute("SELECT id FROM articles WHERE slug = ?", (art.get('slug', ''),)).fetchone()
            if not existing:
                conn.execute(
                    "INSERT INTO articles (title, title_te, content, pdf_url, slug) VALUES (?,?,?,?,?)",
                    (art.get('title',''), art.get('title_te',''), art.get('content',''),
                     art.get('pdf_url',''), art['slug'])
                )
                imported['articles'] += 1
        except Exception:
            pass

    # Restore settings (non-destructive — only update if values provided)
    if 'settings' in data and data['settings']:
        s = data['settings']
        try:
            conn.execute(
                "UPDATE settings SET church_name=?, tagline=?, about=?, contact=?, address=?, social_links=? WHERE id=1",
                (s.get('church_name',''), s.get('tagline',''), s.get('about',''),
                 s.get('contact',''), s.get('address',''), s.get('social_links','{}'))
            )
        except Exception:
            pass
